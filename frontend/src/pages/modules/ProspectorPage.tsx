  ...
  return (
    <div className="flex gap-6 min-h-0">
      <aside className="w-72 shrink-0 space-y-4">
        <div>
          <h1 className="text-xl font-bold text-ink-9">Prospector</h1>
          <p className="text-xs text-ink-5 mt-0.5">Encuentra clientes con senales reales de compra</p>
        </div>

        <div className="flex gap-1 bg-ink-2 rounded-lg p-1 text-xs">
          {([{ id: 'maps', label: 'Local', ok: true }, { id: 'apollo', label: 'Directorio', ok: true }, { id: 'social', label: 'Redes', ok: true }] as { id: Tab; label: string; ok: boolean }[]).map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} className={clsx('flex-1 py-1.5 rounded font-medium transition-colors', tab === t.id ? 'bg-white shadow text-ink-9' : 'text-ink-5 hover:text-ink-7')}>
        ...
        <div className="card p-4 space-y-3">
          {tab === 'maps' && <>
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Que buscar</label>
            ...
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Ciudad / Zona</label>
            ...
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Tu nicho <span className="text-ink-4 font-normal">(para IA)</span></label>
            ...
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Tu producto/servicio</label>
          ...
          {tab === 'apollo' && <>
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Cargos a buscar</label>
              ...
              <p className="text-[10px] text-ink-4 mt-0.5">Separados por coma</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Ubicaciones</label>
              ...
              <p className="text-[10px] text-ink-4 mt-0.5">Separadas por coma</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Industria</label>
          ...
          {tab === 'social' && <>
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Palabras clave</label>
            ...
            <div>
              <label className="block text-xs font-medium text-ink-7 mb-1">Ubicacion / Pais</label>
        ...
        {prospects.length > 0 && (
          <div className="card p-3">
            <p className="text-[11px] font-semibold text-ink-4 uppercase tracking-wide mb-2">Resumen</p>
            <div className="grid grid-cols-3 gap-1 text-center">
              ...
              <div className="bg-kap-50 rounded-lg py-2"><p className="text-lg font-bold text-kap-600">{enPipeline}</p><p className="text-[10px] text-kap-500">Pipeline</p></div>
            </div>
          </div>
        )}
      </aside>
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-ink-6">
            {listLoading ? 'Cargando...' : <>{prospects.length} prospecto{prospects.length !== 1 ? 's' : ''}{searchMutation.isPending && <span className="ml-2 text-kap-500 inline-flex items-center gap-1"><Loader2 size={11} className="animate-spin" /> buscando...</span>}</>}
          </p>
          <button onClick={() => refetch()} className="text-ink-4 hover:text-ink-6 transition-colors"><RefreshCw size={14} /></button>
        </div>

        {!listLoading && prospects.length === 0 && !searchMutation.isPending && (
          <div className="card p-14 text-center space-y-3">
            <Building2 size={44} className="mx-auto text-ink-3" />
            <p className="font-semibold text-ink-5">Sin prospectos todavia</p>
            <p className="text-sm text-ink-4">Completa el formulario y lanza tu primera busqueda.</p>
          </div>
        )}
        ...
        <div className="space-y-2">
          {prospects.map(p => {
            ...
            return (
              <div key={p.id} className={clsx('card overflow-hidden transition-all duration-150', isExpanded && 'ring-1 ring-kap-200')}>
                <div className="p-4 flex items-center gap-3 cursor-pointer hover:bg-ink-1 transition-colors" onClick={() => toggleExpand(p.id, p)}>
                  <ScoreBadge score={p.score} variant="solid" />
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <h3 className="font-semibold text-ink-9 truncate max-w-xs">{p.company_name}</h3>
                      <WebBadge status={p.web_status} />
                      {p.in_pipeline && <span className="text-[10px] px-1.5 py-0.5 bg-kap-100 text-kap-700 rounded-full font-semibold">En pipeline</span>}
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-0.5 text-xs text-ink-5">
                  ...
                  {isExpanded ? <ChevronUp size={16} className="text-ink-4 shrink-0" /> : <ChevronDown size={16} className="text-ink-4 shrink-0" />}
                </div>

                {isExpanded && (
                  <div className="px-4 pb-5 space-y-5 border-t border-ink-2 pt-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                      {p.address && <div className="flex items-start gap-1.5 text-ink-6"><MapPin size={13} className="text-ink-4 mt-0.5 shrink-0" /><span className="break-words">{p.address}</span></div>}
                      {p.phone && <div className="flex items-center gap-1.5"><Phone size={13} className="text-ink-4 shrink-0" /><a href={`tel:${p.phone}`} className="text-ink-7 hover:text-kap-500">{p.phone}</a></div>}
                      {p.website && <div className="flex items-center gap-1.5"><Globe size={13} className="text-ink-4 shrink-0" /><a href={p.website.startsWith('http') ? p.website : `https://${p.website}`} target="_blank" rel="noopener noreferrer" className="text-kap-500 hover:underline truncate max-w-[200px]">{p.website.replace(/^https?:\/\//, '')}</a></div>}
                      {p.source_url && <div className="flex items-center gap-1.5"><Map size={13} className="text-ink-4 shrink-0" /><a href={p.source_url} target="_blank" rel="noopener noreferrer" className="text-ink-6 hover:text-kap-500 flex items-center gap-1">Ver ubicación <ExternalLink size={10} /></a></div>}
                    </div>

                    {p.score_reason && <p className="text-xs text-ink-4 italic bg-ink-1 rounded-lg px-3 py-1.5">{p.score_reason}</p>}

                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <label className="text-xs font-semibold text-ink-7">Notas</label>
                        <span className="text-[11px]">
                          {saveStatus === 'saving' && <span className="text-ink-4 flex items-center gap-1"><Loader2 size={10} className="animate-spin" />guardando...</span>}
                      ...
                      {p.notes_history && p.notes_history.length > 0 && (
                        <div className="mt-2 space-y-1">
                          {p.notes_history.slice(-3).reverse().map((n, i) => (
                            <div key={i} className="text-[11px] text-ink-5 bg-ink-1 rounded-lg px-2.5 py-1.5">
                              <span className="text-ink-4">{new Date(n.created_at).toLocaleString('es-CL', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>
                    ...
                    <div>
                      <p className="text-xs font-semibold text-ink-7 mb-1.5">Alarma de seguimiento</p>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1 border-t border-ink-2">
                      <button className={clsx('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors', p.in_pipeline ? 'bg-ink-2 text-ink-4 cursor-default' : 'bg-kap-500 text-white hover:bg-kap-600')} onClick={() => !p.in_pipeline && llevarAlPipeline(p.id)} disabled={pipelineLoading[p.id] || !!p.in_pipeline}>
                    ...
                    {ia?.texto && (
                      <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 space-y-2.5">
                        ...
                        <p className="text-sm text-ink-8 whitespace-pre-wrap leading-relaxed">{ia.texto}</p>
                      </div>
                    )}
                  </div>
                )}
