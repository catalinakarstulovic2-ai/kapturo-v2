    ...
    <div className="min-h-screen bg-ink-1 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-ink-9">Crear cuenta en Kapturo</h1>
          <p className="text-ink-5 mt-1">Empieza a prospectar en minutos</p>
        </div>

        <div className="card p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-ink-7 mb-1">Nombre de tu empresa</label>
              <input className="input" value={form.companyName} onChange={set('companyName')} required placeholder="Mi Empresa SpA" />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink-7 mb-1">Tu nombre</label>
        ...
        <p className="text-center text-sm text-ink-5 mt-4">
          ¿Ya tienes cuenta?{' '}
          <Link to="/login" className="text-kap-600 font-medium hover:underline">Ingresar</Link>
        </p>
      </div>
    </div>
  )
